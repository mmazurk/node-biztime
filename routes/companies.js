...
router.get("/:code", async function (req, res, next) {
  try {
    ...
    if (invQuery.rows.length === 0) {
      throw new ExpressError("Cant find invoices", 404);
    }
    ...
    if (codeQuery.rows.length === 0) {
      throw new ExpressError("Company has no codes", 404);
    }

    const indCodes = codeQuery.rows.map((item) => item.ind_code);

    return res.json({
      company: companyQuery.rows[0],
      invoices: invQuery.rows,
      industries: indCodes,
    });
  } catch (err) {
    return next(err);
...
router.delete("/:code", async function (req, res, next) {
  try {
    const companyCode = req.params.code;
    const busQuery = await db.query("DELETE FROM companies WHERE code = $1", [
      companyCode,
    ]);
